 ...
 *	tcp-client.c
 *
 *	Source code of TCP messaging sample SOS application (client).
 */
...
*/
	...
	peer_addr.sin_port = htons(10008);		// Port 10004
	memcpy(&peer_addr.sin_addr.s_addr, peer_ip_addr, 4);

	connect(sock, (struct sockaddr*)&peer_addr, sizeof(struct sockaddr_in));
...
//			rv = sendto(sock, msg, msg_len, 0, (struct sockaddr*)&peer_addr, sizeof(struct sockaddr_in));
			...
			from_msg[from_msg_len] = '\0';
			printfxy(0, 16, "%s(): received <<%s>> rv=%d", __func__, from_msg, rv);
		}
	}
